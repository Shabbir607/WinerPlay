      ...
      <form method="post" action="{{route('users.update',$user->id)}}">
        ...
        <img id="holder" style="margin-top:15px;max-height:100px;">
          ...
          @enderror
              ...
              $userRole = $user->role_id; // Assuming $user is an instance of User model with a 'role_id' attribute
              $roles = DB::table('users')->select('role_id')->where('id', $user->id)->get();
          @endphp
          ...
          <div class="form-group">
              ...
              <select name="role" class="form-control">
                  ...
                  @foreach($roles as $role)
                      <option value="{{ $role->role_id }}" {{ $role->role_id == $userRole ? 'selected' : '' }}>
                          {{ ucfirst($role->role_id) }}
                      </option>
                  @endforeach
              </select>
